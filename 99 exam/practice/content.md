...
* Write out the expansion matrix $$X$$ for a quadratic (2nd degree) model.
* Write out the prediction vector $$\mathbf{\hat{y}}$$ using the provided starting values for $$\mathbf{w}$$ and $$b$$.
* Compute the cost for the prediction vector $$\mathbf{\hat{y}}$$ from the previous question.<br>
...
Your line for the model does not need to be exact, but the rough shape should match the type of the model.<br>
